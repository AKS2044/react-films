...
import { fetchDeleteFavouriteFilm, fetchDeleteWatchLaterFilm, fetchGetFavouriteFilm, fetchGetWatchLaterFilm } from '../redux/film/asyncActions';
import { selectFilmData } from '../redux/film/selectors';
import { FavouriteFilmParams } from '../redux/film/types';

const Profile = () => {
    const dispatch = useAppDispatch();
    const isAuth = useSelector(selectIsAuth);
    const { profile, profileStatus, statusAuth } = useSelector(selectLoginData);
    const { filmFavourite, filmWatchLater, deleteFavouriteStatus, deleteWatchLaterStatus } = useSelector(selectFilmData);
    useEffect(() => {
            dispatch(fetchGetFavouriteFilm());
    }, [deleteFavouriteStatus]);

    useEffect(() => {
            dispatch(fetchGetWatchLaterFilm());
    }, [deleteWatchLaterStatus]);

    useEffect(() => {
            dispatch(fetchGetProfile());
    }, []);

    const onClickDeleteFavourite = async (id: number) => {
        const film: FavouriteFilmParams = {filmId: id}
        await dispatch(fetchDeleteFavouriteFilm(film));
    }

    const onClickDeleteWatchLater = async (id: number) => {
        const film: FavouriteFilmParams = {filmId: id}
    ...
    return (
        ...
        <div className='profile__watch'>
            <div className='profile__watch-title'>Избранное({filmFavourite.length}):</div>
            <div className='profile__watch-block'>
                {filmFavourite.map((f) => 
                <div key={f.id} className='profile__watch-block-item'>
                    <img onClick={() => onClickDeleteFavourite(f.id)} className='profile__watch-block-item-cross' src={cross} alt="Удалить" title="Удалить из избранного" />
                    <a href='/'>
                        <img className='profile__watch-block-item-poster' src={`https://localhost:44369/`+ f.pathPoster} alt="Постер" title="Постер" />
                    </a>
                    <div className='profile__watch-block-item-text'>{f.nameFilms}({f.releaseDate})</div>
                </div>)}
            </div>
            <button className='profile__watch-button'>Больше</button>
        </div>
        <div className='profile__watch'>
            <div className='profile__watch-title'>Смотреть позже({filmWatchLater.length}):</div>
            <div className='profile__watch-block'>
                {filmWatchLater.map((f) => 
                    <div key={f.id} className='profile__watch-block-item'>
                        <img onClick={() => onClickDeleteWatchLater(f.id)} className='profile__watch-block-item-cross' src={cross} alt="Удалить" title="Удалить из избранного" />
                        <a href='/'>
                            <img className='profile__watch-block-item-poster' src={`https://localhost:44369/`+ f.pathPoster} alt="Постер" title="Постер" />
                        </a>
                        <div className='profile__watch-block-item-text'>{f.nameFilms}({f.releaseDate})</div>
